...
namespace P06_Sneaking
{
    class Sneaking
    {
        ...
        static void Main()
        {
            ...
            ReadFieldAndSamPos(n, samPosition);
        ...
        private static void MoveEnemy()
        {
            for (int row = 0; row < room.Length; row++)
            {
                for (int col = 0; col < room[row].Length; col++)
                {
                    if (room[row][col] == 'b')
                    {
                        if (row >= 0 && row < room.Length && col + 1 < room[row].Length)
                        {
                            room[row][col] = '.';
                            room[row][col + 1] = 'b';
                            col++;
                    ...
                    {
                        if (row >= 0 && row < room.Length && col - 1 >= 0)
                        {
                            room[row][col] = '.';
                            room[row][col - 1] = 'd';
                        }
        ...
        private static void ReadFieldAndSamPos(int n, int[] samPosition)
        {
